...
public static class TextBoxWordBreakExtensions
{
    ...
    internal static int FindPreviousBoundary(string text, int position)
    {
    ...
    internal static int FindNextBoundary(string text, int position)
    {
    ...
    /// </summary>
    internal static (int Start, int End) GetWordAt(string text, int index)
    {
        if (index < 0 || index >= text.Length)
        {
            return (index, index);
    ...
    // logic in ControlHotkeyExtensions.EnableRemoveWordHotkey.
