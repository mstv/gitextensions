...
{
    public TreeNode Sort(IEnumerable<GitItemStatus> statuses, bool flat, Func<GitItemStatus, TreeNode> createNode)
    {
        ...
        void RemoveParentPath(TreeNode node)
        {
            if (node.Parent?.Tag is string parentPath)
            {
                ...
                if (!flat && node.Tag is FileStatusItem item && item.Item.Path != parentPath)
                {
