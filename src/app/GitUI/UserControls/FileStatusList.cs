    ...
    public sealed partial class FileStatusList : GitModuleControl
    {
        ...
        private static ImageListData CreateImageListData()
        {
            ...
            [
                ...
                (nameof(Images.File), ScaleHeight(Images.File)),
                ...
                .Do(sortType =>
                {
                    _groupBy = sortType switch
                    {
                        ...
                        DiffListSortType.FileStatus or DiffListSortType.FileStatusFlat
                            => new GroupBy(GetItemImageKey, Reverse: true, GetImageKey: k => k, GetLabel: _ => ""),
            ...
            foreach (FileStatusWithDescription i in GitItemStatusesWithDescription)
            {
                ...
                TreeNode diffGroup;
                {
                    ...
                    else
                    {
                        ...
                        IEnumerable<IGrouping<string, GitItemStatus>> grouped = itemStatuses.GroupBy(_groupBy.GetGroupKey);
                        foreach (IGrouping<string, GitItemStatus> group in _groupBy.Reverse ? grouped.Reverse() : grouped)
                        {
                            TreeNode groupNode = _sorter.Sort(group, _flatList, CreateNode);
                            groupNode.Text = _groupBy.GetLabel(group.Key);
                            groupNode.ImageIndex = _imageListData.StateImageIndexMap[_groupBy.GetImageKey(group.Key)];
                            groupNode.SelectedImageIndex = groupNode.ImageIndex;
                            diffGroup.Nodes.Add(groupNode);
                        }

                        if (diffGroup.Nodes.Count == 1)
                        {
