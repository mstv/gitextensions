...
namespace GitCommandsTests
{
    ...
    public sealed class RevisionReaderTests
    {
        ...
        private const string _endOfBody = "1DEA7CC4-FB39-450A-8DDF-762FCEA28B05";
        ...
        [Test]
        public void EndOfBody_should_be_same()
        {
            _endOfBody.Should().Be(RevisionReader.TestAccessor.EndOfBody);
        }

        [Test]
        public void BuildArguments_should_be_NUL_terminated()
        {
            var args = _revisionReader.GetTestAccessor().BuildArgumentsBuildArguments(RefFilterOptions.All, "", "", "", false);

            args.ToString().Should().Contain(" log -z ");
        }

        [Test]
        public void BuildArguments_should_not_contain_endofbody()
        {
            var args = _revisionReader.GetTestAccessor().BuildArgumentsBuildArguments(RefFilterOptions.All, "", "", "", false);

            args.ToString().Should().NotContain(_endOfBody);
        }

        [Test]
        public void BuildArguments_should_contain_endofbody()
        {
            var args = _revisionReader.GetTestAccessor().BuildArgumentsBuildArguments(RefFilterOptions.All, "", "", "", true);

            args.ToString().Should().Contain(_endOfBody);
        }
        ...
        {
            var args = _revisionReader.GetTestAccessor().BuildArgumentsBuildArguments(refFilterOptions, "", "", "", false);
        ...
        {
            var args = _revisionReader.GetTestAccessor().BuildArgumentsBuildArguments(refFilterOptions, "my_*", "my_revision", "my_path", false);
