...
public sealed class TextBoxWordBreakExtensionsTests
{
    ...
    {
        TextBoxWordBreakExtensions.FindNextBoundary(text, position).Should().Be(expected);
    }
    ...
    {
        TextBoxWordBreakExtensions.FindPreviousBoundary(text, position).Should().Be(expected);
    }

    [Test]
    public void FindNextBoundary_clamps_position_past_end_of_text()
    {
        TextBoxWordBreakExtensions.FindNextBoundary("abc", 999).Should().Be(3);
    }

    [Test]
    public void FindPreviousBoundary_clamps_position_past_end_of_text()
    {
        TextBoxWordBreakExtensions.FindPreviousBoundary("abc", 999).Should().Be(0);
    }
    ...
    {
        TextBoxWordBreakExtensions.GetWordAt(text, index).Should().Be((expectedStart, expectedEnd));
    }
}
