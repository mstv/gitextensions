...
public class FileStatusSorterTests
{
    ...
    [Test]
    public async Task Sort_should_first_add_folders_then_files_then_rangediff()
    {
        ...
        TreeNode rootNode = fileStatusSorter.Sort(statuses, status => new TreeNode(status.ToString()) { Tag = new FileStatusItem(firstRev: null, secondRev: new GitRevision(ObjectId.WorkTreeId), status) });
